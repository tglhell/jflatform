...
function parameterUrl () {
	let urlValChk = location.href.substr(location.href.lastIndexOf('=') + 1);
	if (urlValChk == 1) {
		$('body').addClass('cg-preview');
		if ($(window).height() * 5 < $('.cg-preview').height()) {
			$('body').removeAttr('class').addClass('cg-preview position-r');
		}
	}
}
